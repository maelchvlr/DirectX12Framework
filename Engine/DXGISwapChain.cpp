...
namespace Engine
{
	...
	void DXGISwapChain::Initialize(ID3D12Device* pDevice, IDXGIFactory2* pFactory, ID3D12CommandQueue* pCommandQueue, const HWND hwnd, const UINT width, const UINT height)
	{
		...
		ENGINE_EVAL_HR(pDevice->CreateDescriptorHeap(&rtvHeapDesc, IID_PPV_ARGS(mRTVHeap.GetAddressOf())), "Failed to create RTV descriptor heap.");
		...
		ENGINE_EVAL_HR(pFactory->CreateSwapChainForHwnd(pCommandQueue, hwnd, &description, nullptr, nullptr, GetAddressOf()), "Failed to create swap chain.");
	...
	void DXGISwapChain::CreateBuffers()
	{
		...
		for (int i = 0; i < mBufferCount; i++)
		{
			...
			ENGINE_EVAL_HR(Get()->GetBuffer(i, IID_PPV_ARGS(mRenderTargets[i].GetAddressOf())), "Failed creating buffer for swap chain at index : " << i);
	...
	{
		Get()->Present(0,0); //DXGI_PRESENT_ALLOW_TEARING could be used
