...
namespace Engine {
	...
	void D12PipelineState::Initialize(ID3D12Device* pDevice)
	{
		...
		desc.RasterizerState.CullMode = D3D12_CULL_MODE_BACK; // D3D12_CULL_MODE_NONE to draw all the faces (basically you write what you ignore)
		...
		ENGINE_EVAL_HR(pDevice->CreateGraphicsPipelineState(&desc, IID_PPV_ARGS(GetAddressOf())), "Error creating the pipeline state object.");
	...
	void D12PipelineState::InitializeAsTransparent(ID3D12Device* pDevice)
	{
		...
		mRootSignature.Initialize(pDevice);
		mShaders[0].Initialize(L"shaders/pshadow/ShadowVS.hlsl", HLSLShader::ShaderType::VERTEX);
		mShaders[1].Initialize(L"shaders/pshadow/ShadowPS.hlsl", HLSLShader::ShaderType::PIXEL);
		...
		// destination colo: gets multiplied by the inverse source alpha
